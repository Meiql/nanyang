...
public class UserController extends BaseController {
	...
	public ReturnDatas saveorupdate(User user, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		try {
			...
			if(managerOrgNames!=null&&managerOrgIds!=null&&managerOrgIds.length==managerOrgNames.length){
				...
				return new ReturnDatas(ReturnDatas.ERROR,"请选择员工所在部门！");
			}
			...
			if (StringUtils.isBlank(id)) {
				...
				Integer uflag = userService.findUserByName(name);
				if(uflag != 0){
					return new ReturnDatas(ReturnDatas.ERROR,"员工姓名重复，请输入标识字符！");
				}
				...
				//更新员工部门履历
				userResumeService.saveUserOrgResume(managerOrgIds, user.getId());
	...
	public @ResponseBody ReturnDatas checkAccount(HttpServletRequest request,HttpServletResponse response,Model model)throws Exception{
		ReturnDatas datas=ReturnDatas.getSuccessReturnDatas();
		...
		if(u == null){
			...
			return new ReturnDatas(ReturnDatas.ERROR,"账号已存在");
		}
	}
	/**
	 * 员工离职
	...
	public String staffLizhiPre(Model model,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ReturnDatas returnObject = ReturnDatas.getSuccessReturnDatas();
		String id = request.getParameter("id");
		if(StringUtils.isBlank(id)){
			returnObject.setStatus(ReturnDatas.ERROR);
	...
	public @ResponseBody ReturnDatas saffLizhi(Model model,HttpServletRequest request,User user)throws Exception{
