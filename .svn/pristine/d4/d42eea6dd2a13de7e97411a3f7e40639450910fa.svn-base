...
//		.append(") as m where m.userId=u.id ");
      ...
      /*  if(StringUtils.isBlank(qxfinder.getSql())){
		...
		if(CollectionUtils.isNotEmpty(queryForList)){
			for(T ut : queryForList){
				User u = (User) ut;
				UserInfo uf = userInfoService.findUserInfoByUserId(u.getId());
				u.setUserinfo(uf);
			}
		}
		return queryForList;
	...
	@Override
	public void saveUserLizhi(User user) throws Exception {
		if(StringUtils.isEmpty(user.getId())){
			return;
		}
		//修改user主表
		User user1 = super.findById(user.getId(), User.class);
		user1.setActive(ErpStateEnum.userActiveEnum.离职.getValue());
		user1.setEditDate(new Date());
		user1.setEditUser(SessionUser.getUserId());
		user1.setState(userStateEnum.办理离职.getValue());
		super.update(user1);
		//更新user_info
		...
		userInfo.setEndDate(DateUtils.convertString2Date(DateUtils.DATETIME_FORMAT, user.getEndDate()));
		userInfo.setLizhiremark(user.getLizhiremark());
		userInfoService.update(userInfo);
		//更新劳动合同
		workContractService.updateUserLizheWorkContract(user.getId());
		return ;
		
	}

	@Override
	public List<User> finderUserForlizhi() throws Exception {
		Finder finder = new Finder();
		finder.append(" select u.* from "+Finder.getTableName(User.class)+" u ,"+Finder.getTableName(UserInfo.class)+" ui"
				+ " where u.id=ui.userId and u.active=:active and ui.stoppay=:stoppay");
		finder.setParam("active", ErpStateEnum.userActiveEnum.离职.getValue());
		finder.setParam("stoppay", StoppayEnum.正常.getValue());
	...
	@Override
	public List<User> findUserByZichanId(String zcid) throws Exception {
		if(StringUtils.isBlank(zcid)){
			return null;
		}
		Finder finder = Finder.getSelectFinder(User.class);
		finder.append(" where 1 = 1 and id in ( ");
		finder.append("select linguserid from ").append(Finder.getTableName(ZichanLingDetail.class)).append(" where active=0 and lingnum>0 and zcId = :zcId) ");
