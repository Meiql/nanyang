 ...
 */
	...
	public List<TongchouShow> findListTongchou(Page page,
		...
		if(StringUtils.isNotBlank(tongchouShow.getType())){
			finder.append(" and tc.type =:type");
			finder.setParam("type", tongchouShow.getType());
		}
	...
	@Override
	public List<TongchouShow> findListByTypekey(TongchouShow tongchouShow)
			throws Exception {
		if(StringUtils.isEmpty(tongchouShow.getDicttypeId())||StringUtils.isEmpty(tongchouShow.getType())){
			return null;
		}
		Finder finder = new Finder();
		finder.append("select tc.* ,t.`name` dicdataName from "+Finder.getTableName(TongchouShow.class)+" tc ,"+Finder.getTableName(DicData.class)+" t ");
		...
		finder.append(" and tc.type=:type").setParam("type", tongchouShow.getType());
		finder.setParam("typekey", tongchouShow.getDicttypeId()).setParam("showOrhidden", "0");
		finder.append(" order by tc.sortno desc");
