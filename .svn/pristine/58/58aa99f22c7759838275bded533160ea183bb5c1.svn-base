 ...
 */
	...
	public List<WorkContract> findWorkContractByQuery(WorkContract query,Page page)
		...
		if(StringUtils.isNotBlank(query.getContractNo())){
			finder.append(" and c.contractNo like :contractNo ").setParam("contractNo", "%"+query.getContractNo()+"%");
		}
	...
	public List<WorkContract> findJiJiangShixiaoWorkContract() throws Exception {
		...
		Finder finder = Finder.getSelectFinder(WorkContract.class,"w.*,u.name as userName");
		finder.append(" as w left join ").append(Finder.getTableName(User.class)).append(" as u on w.userId = u.id ");
		finder.append("where 1 = 1 ");
		finder.append(" and w.state = :state and w.endDate <= :endDate ").setParam("state", ErpStateEnum.stateEnum.是.getValue());
