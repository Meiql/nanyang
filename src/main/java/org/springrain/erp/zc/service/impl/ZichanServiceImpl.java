...
public class ZichanServiceImpl extends BaseSpringrainServiceImpl implements
	...
	public List<Zichan> findZichanByQuery(Zichan query, Page page)
		...
		if(StringUtils.isNotBlank(query.getGuige())){
			finder.append(" and guige like :guige ").setParam("guige", "%"+query.getGuige()+"%");
		}
	...
	@Override
	public Zichan findZichanByZccode(String zccode) throws Exception {
		if(StringUtils.isBlank(zccode)){
			return null;
		}
		Finder finder = Finder.getSelectFinder(Zichan.class);
		finder.append(" where 1 = 1 and zccode = :zccode ").setParam("zccode", zccode);
		List<Zichan> datas = super.queryForList(finder, Zichan.class);
		if(CollectionUtils.isNotEmpty(datas)){
			return datas.get(0);
		}
		return null;
