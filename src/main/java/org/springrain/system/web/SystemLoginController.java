...
public class SystemLoginController extends BaseController {
	...
	public ReturnDatas sendpwdbymail(HttpServletRequest request, Model model, HttpSession session) {
		...
		try {
			...
			UserInfo ui=userService.findById(u.getId(), UserInfo.class);
			String toemail=ui.getEmail(); 
			if(StringUtils.isBlank(toemail)){
				returnObject.setStatus(ReturnDatas.ERROR);
				returnObject.setMessage("这个账号没有配置邮箱");
				return returnObject;
			}
			...
			data.setTemplateName("backpassword");
			data.setSubject("中云ERP系统密码找回提醒");
			data.setTo(toemail);
			data.send();
			
			returnObject.setData(toemail);
